import { abi as IOtterSwapPoolABI } from '@otterswap/core/artifacts/contracts/interfaces/IOtterSwapPool.sol/IOtterSwapPool.json'
import { Fixture } from 'ethereum-waffle'
import { BigNumber, constants, ContractTransaction, Wallet } from 'ethers'
import { ethers, waffle } from 'hardhat'
import { IOtterSwapPool, IWETH9, MockTimeSwapRouter, TestERC20 } from '../typechain'
...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    pools: [IOtterSwapPool, IOtterSwapPool, IOtterSwapPool]
    ...
    const pools = poolAddresses.map((poolAddress) => new ethers.Contract(poolAddress, IOtterSwapPoolABI, wallet)) as [
      IOtterSwapPool,
      IOtterSwapPool,
      IOtterSwapPool
    ]
  ...
  let pools: [IOtterSwapPool, IOtterSwapPool, IOtterSwapPool]
  ...
  describe('#exactInput', () => {
    ...
    it('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestOtterSwapCallee')
